	...
	if (sentInvitations.length > 0) {
		sentInvitations.forEach(invitation => {
			...
			invitationItem.innerHTML = `
                ...
                <div class="invitation-status">Invitation Sent</div>
                <div class="contributor-toggle">
...
function sendCollaborationInvite(listId, userId, listName) {
	...
	// Schedule auto-accept using the global system
	if (window.autoAcceptSystem) {
		window.autoAcceptSystem.scheduleAutoAccept(
			userId,
			newInvitation.id,
			listId,
			listName,
...
function cancelInvitation(userId, invitationId) {
	...
	// Cancel auto-accept using the global system
	if (window.autoAcceptSystem) {
		window.autoAcceptSystem.cancelAutoAccept(userId, invitationId);
	}
}
